...
Read in CSV
```{r}
# corrected for augsburg meets, miac 2013 -  results through 2017 no conference
minnesota_xc = read.csv("/Users/hannahsonsalla/Desktop/Math/DataScience/Project/miacCC171027.csv")

# corrected for augsburg meets, miac 2013 -  results through 2017 conference
#minnesota_xc = read.csv("/Users/hannahsonsalla/Desktop/Math/DataScience/Project/miacCC171028.csv")
```
...
minnesota_xc_clean <- 
  ...
  #get rid of weather condition in start time 2017 MIAC Conference
  #mutate(start_time = ifelse(grepl("12:00 PM", start_time), "12:00 PM", start_time)) %>%
  #mutate(start_time = ifelse(grepl("11:00 AM", start_time), "11:00 AM", start_time)) %>%
...
Progression of 4 year runners with yearly PRs
...
Men: Plot of all MIAC men four year runners 2013 - 2016
```{r}
miac_four_years_men_plot <- ggplot(four_years_miac_men, aes(x = meet_year, y = year_pr)) + geom_point(alpha = 1/2) + labs(title = "MIAC Men Four Year Runners", subtitle = "Progression of Yearly Personal Bests 2013 - 2016", x = "year",  y = "time (minutes)") + geom_boxplot()

miac_four_years_men_plot
```

Men: Random Individual 3 runners yearly PRs
```{r}
runners <- "Coffey, Joe|Edstrom, Walter|Ryan, Mike"
...
Plot
```{r}
miac_four_years_men_plot2 <- ggplot(men_four_runners_three, aes(x = meet_year, y = year_pr, group = name)) + geom_point(aes(color = name), size = 2) + geom_line(aes(color = name)) + labs(title = "MIAC Men Four Year Runners", subtitle = "Progression of Yearly Personal Bests 2013 - 2016", x = "year",  y = "time (minutes)") 

miac_four_years_men_plot2
```


Men: Top 10 MIAC 2016 Individuals Progression (Yearly PRs)
```{r}
top_10_runners <- "Coffey, Joe|Bugler, Ryan|Newsum, Jovan|Knobbe,Thomas|Jones, Evan|Smith, Trevor|Kuzara, Maxwell|Eggers, Jacob|Dammann, Carston|Edstrom, Walter|Logan,James|Salan, Abdullahi"
men_four_runners_top_10 <-
  four_years_miac_men %>%
  filter(grepl(top_10_runners, name))
```

Plot
```{r}
miac_four_years_men_plot3 <- ggplot(men_four_runners_top_10, aes(x = meet_year, y = year_pr, group = name)) + geom_point(aes(color = name), size = 2) + geom_line(aes(color = name)) + labs(title = "MIAC Championships 2016 Top Ten Men", subtitle = "Progression of Yearly Personal Bests 2013 - 2016", x = "year",  y = "time (minutes)") + scale_color_manual(values=c("palevioletred1", "violetred1", "red", "darkorange2", "goldenrod", "green1", "green4", "turquoise", "blue", "purple", "slategrey")) 

miac_four_years_men_plot3
```

Women
```{r}
four_years_miac_women <-
  miac_xc %>% 
  filter(race_group == "Women") %>%
  filter(meet_kilometers == 6) %>%
  group_by(name, meet_year) %>%
  summarise(year_pr = min(new_time)) %>%
  # want years 2013 - 2016
  filter(!meet_year == 2012 & !meet_year == 2017) %>%
...
Women: Plot of all MIAC men four year runners 2013 - 2016
```{r}
miac_four_years_women_plot <- ggplot(four_years_miac_women, aes(x = meet_year, y = year_pr)) + geom_point(alpha = 1/2) + labs(title = "MIAC Women Four Year Runners", subtitle = "Progression of Yearly Personal Bests 2013 - 2016", x = "year",  y = "time (minutes)") + geom_boxplot()

miac_four_years_men_plot
```

Women: Top 10 MIAC 2016 Individuals Progression (Yearly PRs)
```{r}
top_10_runners2 <- "Irving, Taybri|Meyer, Kimber|Janot,Kayla|McGivern, Bridget|Hoornaert, Jamie|Kosobud, Allison|Trujillo, Claire|Weber, Emma|Hall, Rachel|Nakasaka, Laura|Kirkeide, Courtney|Nipper, Nicole|DeVore, Sarah Jo|Ramsey, Eliza"
women_four_runners_top_10 <-
  four_years_miac_women %>%
  filter(grepl(top_10_runners2, name))
```

Plot
```{r}
miac_four_years_women_plot3 <- ggplot(women_four_runners_top_10, aes(x = meet_year, y = year_pr, group = name)) + geom_point(aes(color = name), size = 2) + geom_line(aes(color = name)) + labs(title = "MIAC Championships 2016 Top Ten Women", subtitle = "Progression of Yearly Personal Bests 2013 - 2016", x = "year",  y = "time (minutes)") + scale_color_manual(values=c("palevioletred1", "violetred1", "red", "darkorange2", "goldenrod", "green1", "green4", "turquoise", "blue", "purple", "slategrey")) 

miac_four_years_women_plot3
```

MIAC Team Placings 2012 - 2016
...
```{r}
men_team_placing_plot <- ggplot(miac_men_team_placings, aes(x = meet_year, y = team_rank, group = team)) + geom_point(aes(color = team), size = 2) + geom_line(aes(color = team)) + labs(title = "MIAC Men Team Placings 2012 - 2016", x = "year",  y = "team rank") + scale_color_manual(values=c("palevioletred1", "violetred1", "red", "darkorange2", "goldenrod", "green1", "green4", "turquoise", "blue", "purple", "slategrey")) 
...
Plot
```{r}
women_team_placing_plot <- ggplot(miac_women_team_placings, aes(x = meet_year, y = team_rank, group = team)) + geom_point(aes(color = team), size = 2) + geom_line(aes(color = team)) + labs(title = "MIAC Women Team Placings 2012 - 2016", x = "year",  y = "team rank") + scale_color_manual(values=c("palevioletred1", "violetred1", "red", "darkorange2", "goldenrod", "green1", "green4", "turquoise", "blue", "steelblue4", "purple", "slategrey")) 
